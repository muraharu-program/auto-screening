...
TOP_N = 6           # 通知する上位銘柄数
MIN_PROB = 0.65      # 最低上昇確率（バックテストで65%帯がPF=15.47と最適）
...
# BUY判定の最低スコア閾値（デフォルト0.3→0.7に引き上げ）
# 2026年4月の実績: BUY判定（スコア0.5〜0.9）でも的中率8%と壊滅的
# スコア0.7未満はCAUTION扱いにして買いをブロックする
MARKET_REGIME_BUY_MIN_SCORE = 0.7
